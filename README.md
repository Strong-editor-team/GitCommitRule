...
- `Область (Scope)` Одно существительное в скобках, определяющее модуль (например: ui, player, net и другие). Не пишите здесь предложения.
...
ВСЕ части коммита указываются только в `Imperative Mood`, где глагол чаще всего выносится на первое место. Примеры:
```
...
В теле коммита должно содержаться более подробное описание каждого из изменений в этом коммите. Элемент начинается с `- ` и с маленькой буквы, в конце точки нет, в `Imperative Mood`.
```
